...
function handleUnexpectedError(error: unknown) {
  ...
  if (error instanceof ApiError) {
    // Return 500 with error.message, error.request, error.response
  }
...
This pattern:
1. Logs the error
2. Checks if it's a DatoCMS `ApiError` (provides structured request/response info)
3. Falls back to `serialize-error` for other errors

Required packages: `@datocms/cma-client` (for `ApiError` type), `serialize-error`.
