...
| **Framework-centric** | Store tags in your app layer (DB or in-memory); webhook handler maps tags → framework cache keys and calls the framework's revalidation API | Next.js (`revalidateTag`), Nuxt, SvelteKit — any framework with its own caching layer |
...
**Note:** The `cda_cache_tags` / `invalidate` event type does not support webhook filters — it always fires for all tag changes. To programmatically create this webhook via the CMA, see `datocms-cma-skill/references/webhooks-and-triggers.md` → "Cache Tags Invalidation Webhook".
...
For the full implementation pattern (replacement `executeQuery`, DB abstraction, webhook route handler), see `datocms-frontend-integrations-skill/references/nextjs.md` → "Cache Tags (Optional)".
