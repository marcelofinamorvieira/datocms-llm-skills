...
1. **Ask for the plugin name** (e.g., "What should we name this plugin?"). This will be used as the folder name.
...
Ask the user the relevant discovery questions. Combine related questions so the conversation stays efficient, and split them into multiple turns only when needed.
...
If you still need to ask about external dependencies or other concerns, use a follow-up question.
...
Based on the user's answers, read the appropriate reference files from the `references/` directory next to this skill file. **Always** read the core architecture reference. Only load what's relevant — don't load everything.
...
| Condition | Route to |
|---|---|
| Querying content with GraphQL for a frontend (not inside a plugin) | **datocms-cda** |
| Standalone scripts, migrations, or webhook/schema management outside a plugin | **datocms-cma** |
