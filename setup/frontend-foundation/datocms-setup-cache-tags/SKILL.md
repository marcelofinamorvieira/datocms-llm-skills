...
**Shared bundle requirement:** This skill reuses references from `datocms-frontend-integrations` and `datocms-cda`. Ensure both companion skills are installed alongside this one so the referenced files are available.
...
2. **Prerequisite: executeQuery wrapper** — Search for an existing `executeQuery` function that wraps `@datocms/cda-client`. This wrapper is the foundation for cache tags.
   ...
   **If executeQuery does not exist, STOP immediately and tell the user:**
   > "The `executeQuery` wrapper must exist before setting up cache tags. Run `datocms-setup-draft-mode` first — it creates the wrapper you need as a foundation."
...
- If the `executeQuery` wrapper does not exist, stop and tell the user to run `datocms-setup-draft-mode` first.
...
**Always load:**
- `../../../skills/datocms-cda/references/draft-caching-environments.md` — for cache tags concepts, webhook payload format, and CDN header table
